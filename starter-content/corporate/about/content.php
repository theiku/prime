<div class="boldgrid-section">
    <div class="container">
        <div class="row" style="padding-bottom: 100px; padding-top: 100px;">
            <div class="col-md-5 col-xs-12 col-sm-5">
                <p><img class="image-shadow" src="<?php $image_path( 'about/about1.png' ) ?>"></p>
            </div>
            <div class="col-md-1 col-sm-1 col-xs-12"></div>
            <div class="col-md-6 col-sm-6 col-xs-12">
                <p style="margin-bottom: 5px; text-transform: uppercase;">Your Best Solution</p>
                <h2 style="margin-top: 0;">What We Do</h2>
                <div class="row bg-editor-hr-wrap">
                    <div class="col-md-12 col-xs-12 col-sm-12">
                        <div>
                            <hr class="bg-hr color1-color bg-hr-15" style="width: 10%; margin-left: 0px; margin-right: auto; margin-top: 0px;">
                        </div>
                    </div>
                </div>
                <p style="margin-top: 35px;">Leveraging sprints with the aim to take this offline. Executing transformation mapping with a goal to build ROI. Taking dark social to in turn think outside the box. Execute cloud computing to, consequently, go viral. Utilise below the fold and then re-target key demographics.</p>
                <p style="margin-top: 35px;">Inform user experience to, consequently, funnel users. Amplifying vertical integration yet be CMSable. Demonstrating customer journeys but target the low hanging fruit.</p>
                <p style="margin-top: 35px;"><a class="button-primary" href="#">Learn More</a></p>
            </div>
        </div>
    </div>
</div>
<div class="boldgrid-section" style="background-image: linear-gradient(to left, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('<?php $image_path( 'about/about2.png' ) ?>'); background-size: cover; background-position: 50% 50%;" data-image-url="https://source.unsplash.com/9l_326FISzk/1920x1080" data-bg-overlaycolor="rgba(33,33,33,0.5)">
    <div class="container">
        <div class="row" style="padding-top: 100px;">
            <div class="col-md-12 col-xs-12 col-sm-12"></div>
        </div>
        <div class="row color5-background-color color-5-text-contrast bg-background-color">
            <div class="col-md-4 col-xs-12 col-sm-4" style="padding: 2em;">
                <p><img class="alignleft" src="<?php $image_path( 'icons/005-value.png' ) ?>" width="61" height="48"></p>
                <h3>Qualified Pros</h3>
                <p>Synchronise branding to in turn disrupt the balance.</p>
            </div>
            <div class="col-md-4 col-xs-12 col-sm-4" style="padding: 2em;">
                <p><img class="alignleft" src="<?php $image_path( 'icons/006-development.png' ) ?>" alt="" width="61" height="48"></p>
                <h3>Performance</h3>
                <p>Repurpose big data and possibly gain traction.</p>
            </div>
            <div class="col-md-4 col-xs-12 col-sm-4" style="padding: 2em;">
                <p><img class="alignleft" src="<?php $image_path( 'icons/007-analysis.png' ) ?>" alt="" width="61" height="61"></p>
                <h3>Years of Experience</h3>
                <p>Leverage bleeding edge and above all, be on brand.</p>
            </div>
        </div>
        <div class="row" style="padding-bottom: 100px;">
            <div class="col-md-12 col-xs-12 col-sm-12"></div>
        </div>
    </div>
</div>
<div class="boldgrid-section">
    <div class="container">
        <div class="row" style="padding-bottom: 100px; padding-top: 100px;">
            <div class="col-md-6 col-sm-6 col-xs-12">
                <p style="margin-bottom: 5px; text-transform: uppercase;">Your Best Solution</p>
                <h2 style="margin-top: 0px;">Why Choose Us</h2>
                <div class="row bg-editor-hr-wrap">
                    <div class="col-md-12 col-xs-12 col-sm-12">
                        <div>
                            <hr class="bg-hr color1-color bg-hr-15" style="width: 10%; margin-left: 0px; margin-right: auto; margin-top: 0px;">
                        </div>
                    </div>
                </div>
                <p style="margin-top: 35px;">Leveraging sprints with the aim to take this offline. Executing transformation mapping with a goal to build ROI. Taking dark social to in turn think outside the box.</p>
                <p style="margin-top: 35px;">Execute cloud computing to, consequently, go viral. Utilise below the fold and then re-target key demographics.</p>
                <div class="row" style="margin-top: 35px;">
                    <div class="col-md-6 col-xs-12 col-sm-12">
                        <h4><i class="fa bg-inserted-icon fa-check-square-o color1-color" style="margin: 0px 15px 0px 0px; font-size: 20px;" aria-hidden="true"><span style="display: none;">&nbsp;</span></i>Transformation</h4>
                        <h4 style="margin-top: 35px;"><i class="fa bg-inserted-icon fa-check-square-o color1-color" style="margin: 0px 15px 0px 0px; font-size: 20px;" aria-hidden="true"><span style="display: none;">&nbsp;</span></i>Operations &amp; Robotics</h4>
                    </div>
                    <div class="col-md-6 col-xs-12 col-sm-12">
                        <div>
                            <h4><i class="fa bg-inserted-icon fa-check-square-o color1-color" style="margin: 0px 15px 0px 0px; font-size: 20px;" aria-hidden="true"> </i>Information Technology</h4>
                            <h4 style="margin-top: 35px;"><i class="fa bg-inserted-icon fa-check-square-o color1-color" style="margin: 0px 15px 0px 0px; font-size: 20px;" aria-hidden="true"> </i>Unlimited Possibilites</h4>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-1 col-sm-1 col-xs-12"></div>
            <div class="col-md-5 col-xs-12 col-sm-5">
                <p><img src="<?php $image_path( 'about/about3.png' ) ?>" class="image-shadow"></p>
            </div>
        </div>
    </div>
</div>
<div class="boldgrid-slider boldgrid-slider-wrap">
  <div class="boldgrid-section">
      <div class="container">
          <div class="row" style="padding-top: 100px; padding-bottom: 100px;">
              <div class="col-md-4 col-xs-12 col-sm-5">
                  <p><img src="<?php $image_path( 'about/about4.png' ) ?>" class="image-shadow"></p>
              </div>
              <div class="col-md-8 col-sm-7 col-xs-12">
                  <p class="h4">Leveraging sprints with the aim to take this offline. Executing transformation mapping with a goal to build ROI. Taking dark social to in turn think outside the box.</p>
                  <p class="h4" style="text-align: right; margin-top: 75px;">Starr Hudson</p>
                  <p style="text-align: right;" class="color1-color">Chief Executive Officer | Box Em Up</p>
              </div>
          </div>
      </div>
  </div>
  <div class="boldgrid-section">
      <div class="container">
          <div class="row" style="padding-top: 100px; padding-bottom: 100px;">
              <div class="col-md-4 col-xs-12 col-sm-5">
                  <p><img src="<?php $image_path( 'about/about4b.png' ) ?>" class="image-shadow"></p>
              </div>
              <div class="col-md-8 col-sm-7 col-xs-12">
                  <p class="h4">. Engaging customer experience so that as an end result, we be CMSable. Leverage below the fold and finally gain traction. Generating bleeding edge and creating actionable insights.</p>
                  <p class="h4" style="text-align: right; margin-top: 75px;">Teresa Oliver</p>
                  <p style="text-align: right;" class="color1-color">Brand Expert | Mantell Design</p>
              </div>
          </div>
      </div>
  </div>
  <div class="boldgrid-section">
      <div class="container">
          <div class="row" style="padding-top: 100px; padding-bottom: 100px;">
              <div class="col-md-4 col-xs-12 col-sm-5">
                  <p><img src="<?php $image_path( 'about/about4c.png' ) ?>" class="image-shadow"></p>
              </div>
              <div class="col-md-8 col-sm-7 col-xs-12">
                  <p class="h4"> Generating best in class in order to improve overall outcomes. Inform outside the box thinking and finally target the low hanging fruit.</p>
                  <p class="h4" style="text-align: right; margin-top: 75px;">Leslie Boone</p>
                  <p style="text-align: right;" class="color1-color">Support Staff | Gerwyn Financial</p>
              </div>
          </div>
      </div>
  </div>
</div>
<div class="boldgrid-section color4-background-color color-4-text-contrast bg-background-color">
    <div class="container">
        <div class="row" style="padding-top: 100px; padding-bottom: 35px;">
            <div class="col-md-4 col-sm-4 col-xs-12">
                <p style="margin-bottom: 5px; text-transform: uppercase;">Meet the Professionals</p>
                <h2 style="margin-top: 0px;">Team</h2>
                <div class="row bg-editor-hr-wrap">
                    <div class="col-md-12 col-xs-12 col-sm-12">
                        <div>
                            <hr class="bg-hr color1-color bg-hr-15" style="width: 10%; margin-left: 0px; margin-right: auto; margin-top: 0px;">
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-1 col-sm-1 col-xs-12"></div>
            <div class="col-md-7 col-xs-12 col-sm-7">
                <p style="margin-top: 35px;">Target user stories so that we maximise share of voice. Grow cloud computing with the aim to take this offline. Lead integrated tech stacks and above all, target the low hanging fruit.</p>
            </div>
        </div>
        <div class="row" style="padding-bottom: 100px;">
            <div class="col-md-3 col-xs-12 col-sm-6">
                <div class="text-center bg-box bg-box-square bg-box-cover color-5-text-contrast color5-background-color" style="padding: 0.5em 0.5em 1em; margin: 0px -13px;">
                    <p><img src="<?php $image_path( 'about/about5.png' ) ?>"></p>
                    <h3>Nathan Counsel</h3>
                    <p class="color1-color">Chief Executive Officer</p>
                </div>
            </div>
            <div class="col-md-3 col-xs-12 col-sm-6">
                <div class="text-center bg-box bg-box-square bg-box-cover color-5-text-contrast color5-background-color" style="padding: 0.5em 0.5em 1em; margin: 0px -13px;">
                    <p><img src="<?php $image_path( 'about/about6.png' ) ?>"></p>
                    <h3>Morten Iverson</h3>
                    <p class="color1-color">Chief Financial Officer</p>
                </div>
            </div>
            <div class="col-md-3 col-xs-12 col-sm-6">
                <div class="text-center bg-box bg-box-square bg-box-cover color-5-text-contrast color5-background-color" style="padding: 0.5em 0.5em 1em; margin: 0px -13px;">
                    <p><img src="<?php $image_path( 'about/about7.png' ) ?>"></p>
                    <h3>Julie Grave</h3>
                    <p class="color1-color">Information Technology Officer</p>
                </div>
            </div>
            <div class="col-md-3 col-xs-12 col-sm-6">
                <div class="text-center bg-box bg-box-square bg-box-cover color-5-text-contrast color5-background-color" style="padding: 0.5em 0.5em 1em; margin: 0px -13px;">
                    <p><img src="<?php $image_path( 'about/about8.png' ) ?>"></p>
                    <h3>Hans Paulsen</h3>
                    <p class="color1-color">Operations Director</p>
                </div>
            </div>
        </div>
    </div>
</div>
